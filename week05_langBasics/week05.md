...
> #### 示例：表单
> `examples/sec01_html/01_form.html` 演示了一个基本的表单示例，现在很少有应用会直接使用这种最原始的表单。更多的应用选择直接通过 JavaScript 来负责表单的交互。同时，包括`React`在内的各类前端框架，也都有很多组件库内置了组件化设计的表单元素。

> #### 示例：表格
> `examples/sec01_html/02_table.html` 演示了几个基本的表格。尽管在网站设计中已经很少见了，但表格依旧是用于实现电子邮件复杂布局的一个主流选择。
